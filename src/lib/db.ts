...
const SORT_SQL: Record<GenerationSort, string> = {
  ...
  duration_desc:
    "COALESCE(audio_duration_sec, duration_sec) DESC, created_at DESC",
};
...
export function listGenerations(query: ListGenerationsQuery = {}): Generation[] {
  ...
  if (q) {
    ...
    clauses.push(
      `(title LIKE ? ESCAPE '\\' OR prompt LIKE ? ESCAPE '\\' OR IFNULL(lyrics, '') LIKE ? ESCAPE '\\' OR IFNULL(style, '') LIKE ? ESCAPE '\\')`
    );
  ...
  const sql = `SELECT * FROM generations ${where} ORDER BY ${SORT_SQL[sort]}`;
